-------------------------------------------------------------------------------
--
-- File Name: test_program_pkg.vhd
-- Author: Leon Hiemstra
--
-- Title: CDP1802 test program (shared RAM contents)
--
-- License: MIT
--
-- Description:
--   The instruction sequence exercising most of the instruction set,
--   originally embedded directly in ram.vhd. Factored out here so both
--   ram.vhd (single-port, used by cdp18/cs1800 for simulation) and any
--   board-level memory (e.g. a dual-port BRAM for external program
--   loading) can share one definition instead of keeping two literal
--   copies of the same program in sync by hand.
--
-------------------------------------------------------------------------------
...
PACKAGE test_program_pkg IS
  ...
  CONSTANT c_test_program : t_mem_array(0 TO 275) := (
