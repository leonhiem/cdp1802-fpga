...
#       muxaddr                  the core against a REAL memory card: the
#                                multiplexed ADDR bus + a 4042 latched by
#                                TPA, no A_full (TODO 2.5/6)
...
QUICK=(cdp18 cs1800 memory console modes pintiming muxaddr cdp18_sync board isa dma alu random)
...
for t in "${TARGETS[@]}"; do
  case "$t" in
    ...
    muxaddr)  run_check tb_cdp1802_mux_addr "$TB/tb_cdp1802_mux_addr.vhd" ;;
